...
class Arsip_model extends CI_Model
{
    ...
    public function periode($tglawal, $tglakhir)
    {
        $this->db->where('tglsurat >=',$tglawal);
        $this->db->where('tglsurat <=',$tglakhir);
        // $query = $this->db->get('arske');
        $query = $this->db->query("select * from arske where tglsurat between '$tglawal' and '$tglakhir'");
        return $query->result_array();
    }

    public function perimasuk($tglawal, $tglakhir)
    {
        $this->db->where('tglsurat >=',$tglawal);
        $this->db->where('tglsurat <=',$tglakhir);
        // $query = $this->db->get('arske');
        $query = $this->db->query("select * from arsma where tglteri between '$tglawal' and '$tglakhir'");
        return $query->result_array();
    }
